...
export default function PilotoCrud() {
  ...
  const [pilotoEnEdicionId, setPilotoEnEdicionId] = useState<string | null>(null);
  
  const handleGuardarPiloto = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.mensaje || "Error al guardar piloto.");

      if (pilotoEnEdicionId) {
        setPilotos((prev) =>
          prev.map((a) => (a._id === pilotoEnEdicionId ? data.usuario : a))
        );
      } else {
        // crear nuevo
        setPilotos((prev) => [...prev, data.usuario]);
      }

      // resetear todo
      setShowModal(false);
      setModoEdicion(false);
      setPilotoEnEdicionId(null);
      setErrores([]);
      setFormData({
        nombre: "",
        apellido1: "",
  ...
  const confirmarEliminacion = async () => {
    ...
    try {
      ...
      setPilotos((prev) => prev.filter((a) => a._id !== pilotoAEliminar._id));
  ...
  return (
    <AdminLayout current="Pilotos">
      <div className="flex justify-between">
        <h1 className="text-2xl font-bold mb-6 text-gray-800">Pilotos</h1>
        <button
          ...
          >
            <div>
              <h2 className="text-xl font-semibold mb-2">
                {piloto.nombre + " " + piloto.apellido1}
              </h2>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div>
                  <p className="font-bold">Nombre Completo</p>
                  <p>{piloto.nombre + " " + piloto.apellido1 + " " +  piloto.apellido2}</p>
                </div>
                <div>
              ...
              <button
                ...
                onClick={() => {
                  setFormData({
                    ...
                    apellido1: piloto.apellido1,
                    apellido2: piloto.apellido2,
                  ...
                  setErrores([]);
                  setPilotoEnEdicionId(piloto._id);
                  setModoEdicion(true);
                  setShowModal(true);
                }}
              >
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 text-gray-800">
          <div className="bg-white p-6 rounded-lg w-full max-w-md text-gray-800">
            <h2 className="text-xl font-bold mb-4 ">Agregar Piloto</h2>
            {errores.length > 0 && (
      ...
      {mostrarConfirmacion && pilotoAEliminar && (
        <div className="fixed inset-0 bg-[rgba(0,0,0,0.3)] backdrop-blur-sm flex justify-center items-center z-50 text-gray-800">
          <div className="bg-white p-6 rounded-lg w-full max-w-sm text-center border border-gray-800">
            <h2 className="text-xl font-bold mb-2">¿Desea eliminar a este Piloto?</h2>
            <p className="text-sm text-gray-600 mb-6">
              Se eliminará permanentemente: <br />
              <strong>{pilotoAEliminar.nombre} {pilotoAEliminar.apellido1}</strong>
            </p>
